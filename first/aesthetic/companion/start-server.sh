...
if [[ -n "$PROJECT_DIR" ]]; then
  ...
  # One server per project, tracked ABOVE the session directory. The per-session
  # pid file below cannot do this job: the session id is minted fresh on every
  # run, so the file it names has never existed and the "kill any existing
  # server" step has never killed anything. Every restart left the old server
  # running on its own port, and because each process keeps its own websocket
  # client set, two browsers landing on two ports could never see each other's
  # scores -- while both kept appending to the same ledger, so the divergence
  # was invisible on disk.
  OWNER_PID_FILE="${PROJECT_DIR}/.superpowers/brainstorm/.server.pid"
else
...
# Retire the project's previous server before binding a new one, and WAIT for it
# to actually exit -- otherwise the port is still held, the new process falls
# back to a different one, and the project ends up with two live servers that
# cannot see each other's clients.
if [[ -n "$OWNER_PID_FILE" && -f "$OWNER_PID_FILE" ]]; then
  prev_pid="$(cat "$OWNER_PID_FILE" 2>/dev/null || true)"
...
# Kill any existing server recorded for this session directory
if [[ -f "$PID_FILE" ]]; then
