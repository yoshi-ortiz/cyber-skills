...
admits: server.cjs, start-server.sh, install.sh, frame-template.html, helper.js -- the browser surface that carries clicks to the durable ledger
...
`start-server.sh` cds to this directory and `server.cjs` reads `frame-template.html`
and `helper.js` from `__dirname`. SKILL.md launches `companion/start-server.sh`, so
both have to live here, not only under brainstorming/scripts/. A start without
`helper.js` dies in 5 seconds and never binds the port.
