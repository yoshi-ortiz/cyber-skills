...
It is the one registry. `release.py` asks it before it publishes a channel, so
a release proves what a local run proves rather than deciding for itself.

Test gates are found, never listed. B-023 was a test file that passed when
invoked and was executed by nothing, because the gate list named files by hand
and one file was in no list. Two conventions are allowed and the runner tells
them apart by reading the file: a `test_*.py` with a `TestCase` is discovered
with its directory, and one without puts its assertions in a `test()` behind
`__main__` and is run as a script. Either is fine. A file that is neither --
one whose assertions run under no invocation the runner makes -- is the third
convention, and it is the one that ships untested code.

`loanwords.py` owns both language boundaries. It keeps interface nouns in
English across README translations and derives semantic alias rules from the
root `UBIQUITOUS_LANGUAGE.md`. Repo-Dev contract prose opts into the relevant
canonical term with an explicit vocabulary block, so homonyms in another
domain do not become false failures.

`fog.py` holds the list once so `publish.py` and `check_publication.py` cannot
disagree about generic fog. The catalog holds `ALPHA_SKILLS`: a skill named there is
...
`index_gate.py` reads each catalog record's channel and checks the README against it:
