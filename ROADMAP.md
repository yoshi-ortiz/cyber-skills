...
| R-36 | `TODO` | Write `build`, `land`, and `fix` | `build` is a router over `ponytail` and `tdd` (`clean-code`, `qa-tests`, `pre-release`). `land` owns the burndown state machine (`asap-burndown`, `deployed-release`). `fix` is a bare on-ramp over `diagnosing-bugs`, bare because breakage arrives from outside the sequence, and it owns **the rail**: `fix-context-derail` plus the `rail` alias. Repairing a derailed context is a write and the same reflex as repairing code; `check` only reads and reports. |
