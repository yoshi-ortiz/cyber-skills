...
| **`first`** | § Take note<br>§ Design | `f` ≡ `first`<br>`f take note` →§ Take note<br>`f design` →§ Design | Genesis aligns owner intent to requirements and a bounded Item. Sketches explore creative directions. Aesthetic stays outside this route until it passes an unslop review and the owner re-enables it. |
...
| **`fix`** | § Fix the code<br>§ Fix the rail<br>§ Restore missing agent tools<br>§ Resume and report | `fix` bare<br>`fix-context-derail` →§ Fix the rail<br>`rail` `unstick` ≡ | Tokens QA owns `shot-audit`, correction bundles and the session compass. Kit owns managed collection setup. Fix repairs context and missing capabilities, verifies recovery and resumes the goal. |
