...
function handleUpgrade(req, socket) {
  ...
  // Hand the newcomer the current picture. Broadcasting only future decisions
  // leaves every browser starting from whatever snapshot was baked into the
  // page it loaded, so two browsers opened at different moments disagree until
  // somebody happens to click -- which reads, correctly, as "not synced".
  try {
...
function handleMessage(text) {
  ...
  if (event && (event.choice || event.element || event.sentiment || event.verdict || event.reset)) {
    const stamped = { ...event, session: path.basename(SESSION_DIR) };
    const line = JSON.stringify(stamped) + '\n';
    fs.appendFileSync(path.join(STATE_DIR, 'events'), line);
    // Also append to a session-independent ledger. Each restart mints a new
    // session directory, so a session-scoped file silently orphans every
    // decision the moment the server is restarted.
    try {
      fs.appendFileSync(path.join(path.dirname(SESSION_DIR), 'decisions.jsonl'), line);
    } catch (e) {
      console.error('Failed to append durable decision:', e.message);
    }
    // Fan the decision out to every other open view of this screen. Without
    // this the server is write-only: two browsers scoring the same screen each
    // kept a private picture of it, and only the file on disk knew the truth.
    // Browser-local storage cannot close this gap -- it is per profile, so a
    // second browser (or a second machine) never sees the first one's clicks.
    // `from` lets a client ignore the echo of its own message.
    broadcast({ type: 'dh-signal', event: stamped });
  }
}

// Replay the durable ledger into "where does every element stand right now".
